...
本章圍繞一個核心問題：怎麼判斷 Agent 真的變好了？從搭建可復現的測試環境，到設計經得起洩漏考驗的資料集，到讓 LLM 擔任評判者，最後到用評估結果驅動模型選型和迭代——這條鏈路的每個環節都會影響結論的可信度。模型選型還應比較不同資源預算下的能力成長曲線，而不能只看單點成績。生產級 Agent 的評估不是偶爾做一次的考試，而是嵌入到每個產品決策中的持續驗證。
